...
```
$ kubectl get svc gateway -n fridgy
```
...
```
$ kubectl scale deployment <app-name> --replicas <replica-count> -n fridgy
```
...
```
$ kubectl set image deployment/<app-name>-app <app-name>=<new-image>  -n fridgy
```
...
```
$ kubectl expose service jhipster-registry --type=NodePort --name=exposed-registry -n fridgy
```

and explore the details using

```
$ kubectl get svc exposed-registry -n fridgy
```

For scaling the JHipster registry, use

```
$ kubectl scale statefulset jhipster-registry --replicas 3 -n fridgy
```
